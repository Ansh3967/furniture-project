...
const Cart = () => {
  ...
  const getItemImage = (furnitureId: string, fallbackImages?: string[]) => {
    const images =
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="text-center">
            ...
            <p className="text-muted-foreground mb-8">
              Discover our amazing furniture collection and find something you
              love!
            </p>
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-6">
                  {state.cart.map((item) => (
                    <div
                      key={`${item.furniture.id}-${item.type}`}
                      className="flex gap-4">
                      <div className="w-24 h-24 rounded-lg overflow-hidden bg-muted">
                        <img
                          src={getItemImage(
                            item.furniture.id,
                            item.furniture.images
                          )}
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-2 mb-3">
                          <Badge
                            variant={
                              item.type === "sell" ? "default" : "secondary"
                            }>
                            {item.type === "sell" ? (
                              <>
                        ...
                        <div className="flex justify-between items-center">
                          ...
                          <div className="text-right">
                            <div className="font-semibold text-lg">
                              ...
                              {(item.type === "sell"
                                ? item.furniture.price!
                                : item.furniture.rentPrice!) * item.quantity}
                              {item.type === "rent" && "/mo"}
                            </div>
                            {item.type === "rent" && item.furniture.deposit && (
                              <div className="text-sm text-muted-foreground">
                                +₹{item.furniture.deposit * item.quantity}{" "}
                                deposit
                              </div>
                            )}
                          </div>

...
import { Search, Filter, Grid, List, Star, Heart, ShoppingCart, Calendar } from 'lucide-react';
...
const FurnitureListing = () => {
  ...
  const furnitureWithImages = useMemo(() => {
    return state.furniture.map(item => ({
      ...item,
      images: item.id === '1' ? [sofaImage] 
             : item.id === '2' ? [deskImage]
             : item.id === '3' ? [chairImage]
             : ['/placeholder.svg']
    }));
  }, [state.furniture]);
  ...
  const filteredFurniture = useMemo(() => {
    let filtered = furnitureWithImages.filter(item => {
      // Search filter
      if (state.searchQuery && !item.title.toLowerCase().includes(state.searchQuery.toLowerCase()) && 
    ...
    return filtered;
  }, [furnitureWithImages, state.searchQuery, state.selectedCategory, state.typeFilter, state.priceRange, sortBy]);
  ...
  const categories = ['All', 'Sofa', 'Chair', 'Table', 'Bed', 'Desk', 'Storage', 'Decor'];
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-primary mb-2">Furniture Collection</h1>
          <p className="text-muted-foreground">Discover premium furniture for your home and office</p>
        </div>
            ...
            <Card className="p-6">
              ...
              <div className="space-y-2 mb-6">
                ...
                <Select 
                  ...
                  onValueChange={(value: 'all' | 'sell' | 'rent') => dispatch({ type: 'SET_TYPE_FILTER', payload: value })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div className="space-y-4 mb-6">
                ...
                <Slider
                  ...
                  onValueCommit={(value) => dispatch({ type: 'SET_PRICE_RANGE', payload: value })}
                  max={5000}
                  min={0}
                  step={50}
                  className="w-full"
          ...
          <div className="lg:col-span-3">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
              <p className="text-muted-foreground">
                Showing {filteredFurniture.length} of {furnitureWithImages.length} items
              </p>
            ...
            <div className={`grid gap-6 ${viewMode === 'grid' ? 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3' : 'grid-cols-1'}`}>
              {filteredFurniture.map((furniture) => (
                <Card key={furniture.id} className="group hover:shadow-medium transition-shadow">
                  <CardHeader className="p-0">
                    <div className="relative overflow-hidden rounded-t-lg">
                      ...
                      >
                        <Heart 
                          className={`w-4 h-4 ${state.wishlist.includes(furniture.id) ? 'fill-red-500 text-red-500' : 'text-gray-600'}`} 
                        />
                      </Button>
                      {furniture.type === 'both' && (
                  ...
                  <CardContent className="p-4">
                    ...
                    <div className="flex items-center gap-2 mb-3">
                      <div className="flex items-center">
                        ...
                        <span className="text-sm font-medium ml-1">{furniture.rating}</span>
                      </div>
            ...
            {filteredFurniture.length === 0 && (
              <div className="text-center py-12">
                <h3 className="text-xl font-semibold mb-2">No furniture found</h3>
